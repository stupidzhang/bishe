<template>
  <div>
    <li
      v-if="nodata"
      class="padding-top20 padding-bottom40 text-align-center color-999"
    >
      暂无数据
    </li>
    <ul class="padding-left20">
      <li
        ...
        :key="index"
        class="width100 padding20X border-bottom1 flex"
        @tap="clickItem(item)"
      >
        <div class="posterSize itemflex-00auto relative">
          <image :src="item.image" class="posterSize" :lazy-load="true"></image>
         
        </div>
        <div class="width100 paddingX20">
          <div class="flex-align-spacebetween">
            <p class="flex-align" style="width: 65%">
              <span class="color-333 font-size4 line-ellipsis">{{
                item.name
              }}</span>
            </p>
            <p
              class="padding-left10 itemflex-00auto color-yellow"
              style="max-width: 35%"
            >
              <span>{{ item.city }}</span>
            </p>
